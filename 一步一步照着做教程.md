...
展开 **TLS** 设置：
- ✅ **No TLS Verify**（启用关闭 TLS 验证）
- ✅ **HTTP2 connection**（启用 HTTP/2 连接）
![设置TLS](https://raw.githubusercontent.com/oyz8/action/refs/heads/main/img/Tunnel.png)
...
## 六、生成 NZ_UUID
UUID 是 Agent 的唯一标识符
1. 在线生成访问:https://www.uuidgenerator.net/
2. 复制生成的 UUID（格式如：
```
...
| `GITHUB_BRANCH` | ✅ | 分支名 |
| `ZIP_PASSWORD` | ✅ | 备份密码 |
| `NZ_UUID` | ✅ | 是 Agent 的 UUID，用于探针上报 |
| `NZ_TLS` | ❌ | 默认 `true` |
| `DASHBOARD_VERSION` | ❌ | 默认 `latest` |
| `PROJECT_URL` | ❌ | 示例值 `https://xxx.hf.space` 设置即启用保活，不设置则跳过 |
...
#### 查看抱脸URL
![查看抱脸URL](https://raw.githubusercontent.com/oyz8/action/refs/heads/main/img/PROJECT_URL.png)
